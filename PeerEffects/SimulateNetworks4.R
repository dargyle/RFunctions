...
notsurveyed <- controls.net$hhSurveyed==0
missing.links <- outer(notsurveyed,notsurveyed,'+')==2
net[missing.links] <- NA

miss.net <- network(net,vertex.attr=controls.net,matrix.type='adjacency',directed=FALSE) 
...
sim.fit <- simulate(fit,nsim=num.sims,
                    basis=miss.net,
                    control = control.simulate.ergm(burnin=50000,
                                                    MCMC.interval = 10000),
